...
import seaborn as sns # 원본 코드에 있었으나, 실제 사용은 확인 필요. 일단 유지.

# ----------------------------------------------------
# 1. ConsumptionAgent 클래스 정의
# ----------------------------------------------------
class ConsumptionAgent:
    """
    고객 소비 데이터를 분석하고, KNN 모델로 군집을 예측하며, Ollama LLM을 사용해 
    최종 보고서를 생성하는 역할을 수행하는 에이전트 클래스입니다.
    """
    def __init__(self, knn_path, scaler_path, profile_path, data_path, ollama_model_name="qwen3:8b"):
        try:
            # 1. 자산 로드: 폴더 구조 (data/, models/) 반영
            ...
            # 3. 한글 폰트 설정 (Colab 환경에서 가져온 부분, 로컬 환경에서 오류 시 주석 처리 필요)
            # plt.rcParams['font.family'] = 'NanumGothic'
            # plt.rcParams['axes.unicode_minus'] = False

            print("✅ Agent 초기화: 모델, 스케일러, 프로파일 로드 완료.")
        except Exception as e:
            # 파일 경로 오류 발생 시 명확하게 안내
            raise FileNotFoundError(f"❌ 필수 파일 로드 실패: {e}. 'data/'와 'models/' 폴더 경로를 확인하세요.")
    ...
    def generate_final_report(self, nickname, analysis_data):
        ...
        try:
            # Ollama 서버 통신 (로컬 PC의 http://localhost:11434)
            response = requests.post("http://localhost:11434/api/generate", json=payload, timeout=60)
            response.raise_for_status()
            final_report = response.json()['response'].strip()
            return final_report
        except requests.exceptions.RequestException as e:
            return f"❌ Ollama 통신 오류: Ollama 서버가 실행 중인지 확인하세요. 오류: {e}"

    # Task 3: 군집 시각화 함수 (로직은 원본 코랩과 동일)
...
if __name__ == "__main__":
    ...
    # Colab에서 Ollama 설치 및 서버 실행을 위한 비동기 코드는 제거됨. 
    # VS Code 환경에서는 Ollama 서버를 별도로 수동 실행해야 합니다.
    
    try:
        # 1. Agent 초기화
        agent = ConsumptionAgent(KNN_MODEL_PATH, SCALER_MODEL_PATH, CLUSTER_PROFILE_PATH, FINAL_DATA_PATH, ollama_model_name=AGENT_OLLAMA_MODEL)
        ...
        print(f"\n❌ 오류: 필수 파일 경로를 찾을 수 없습니다. {e}")
        print("💡 'data/'와 'models/' 폴더에 파일이 올바르게 배치되었는지 확인하세요.")
    except Exception as e:
        print(f"\n❌ 파이프라인 실행 중 오류 발생: {e}")