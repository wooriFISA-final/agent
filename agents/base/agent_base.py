...
class AgentBase(ABC):
    ...
    async def execute_multi_turn(self, state: AgentState) -> AgentState:
        ...
        while not StateBuilder.is_max_iterations_reached(state):
            ...
            if decision.action == AgentAction.USE_TOOL:
                logger.info(f"🔧 [DEBUG] Executing tool: {decision.tool_name}")
                logger.info(f"🔧 [DEBUG] Tool arguments: {decision.tool_arguments}")
                state = await self._execute_tool_action(state, decision)
                logger.info(f"🔧 [DEBUG] Tool execution completed. Current iteration: {state.get('iteration')}/{self.max_iterations}")
                logger.info(f"🔧 [DEBUG] Messages in state: {len(state.get('global_messages', []))}")
                continue
    ...
    async def _make_decision(
        ...
        try:
            ...
            # LLM 호출 - JSON 기반 tool calling 사용 (Bedrock native 비활성화)
            formatted_messages = self._convert_messages_to_dict(messages)
            ...
            response = await asyncio.to_thread(
                ...
                history=formatted_messages,
                # tool_config=bedrock_tool_config,  # 🚨 비활성화: Bedrock native tool calling
