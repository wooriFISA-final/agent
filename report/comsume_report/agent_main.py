...
class ConsumptionAgent:
    def __init__(self, knn_path, scaler_path, profile_path, data_path, ollama_model_name="qwen3:8b"):
        try:
            ...
            # 3. 한글 폰트 설정 (Windows 환경을 고려하여 Nanum 대신 Malgun Gothic 선호)
            # Colab이 아닌 VS Code 로컬 환경에서는 Nanum 폰트 대신 시스템 폰트를 지정하는 것이 안정적입니다.
    ...
    def generate_final_report(self, nickname, analysis_data):
        ...
        try:
            ...
            return f"❌ Ollama 통신 오류: 로컬 서버 연결 실패. 오류: {e}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("💡 Ollama 서버가 'ollama run qwen3:8b' 상태로 실행 중인지 확인하세요.")